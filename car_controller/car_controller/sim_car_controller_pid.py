...
from simple_pid import PID  # Add this import
...
class SimCarController(Node):
    def __init__(self):
        ...
        self.subscription_error = self.create_subscription(Float32, '/error', self.error_callback, 10)
        ...
        self.get_logger().info('SimCarController (PID) has been started.')

        # PID controller for lateral control (tune Kp, Ki, Kd as needed)
        self.pid = PID(Kp=0.5, Ki=0.0001, Kd=2.5, setpoint=0.0)
        self.pid.output_limits = (-self.max_steering_angle, self.max_steering_angle)
    ...
    def timer_callback(self):
        ...
        # Lateral error is the y-coordinate of the target point in the local frame (meters)
        lateral_error = self.latest_error if self.latest_error else 0

        # PID controller computes steering angle
        steering_angle = self.pid(lateral_error)
        steering_angle = np.clip(steering_angle, -self.max_steering_angle, self.max_steering_angle)

        # Publish the steering angle
        self.cmd_publisher.publish(self.build_ackermann_msg(self.car_velocity, steering_angle))
        self.get_logger().info(f'Published to /cmd: speed={self.car_velocity:.2f} km/h, steering_angle={steering_angle:.4f}, lateral_error={lateral_error:.4f}')
